...
var getCachedConsumer = function(name, cb) {
    ...
    cache.get(name, function(err, consumer){
        if (!!err) {
            logger.trace({err:err}, 'error retrieving consumer from cache');
            return cb(err);
        }

        logger.trace({consumer: consumer}, 'consumer returned from cache');
        if (consumer === undefined) {
            logger.trace('consumer "' + name + '" not in cache, reload from ZooKeeper');
            return getZkConsumerTopics(name).then(function(consumer) {
                logger.trace({consumer:consumer}, 'saving consumer "' + name + '" to cache');
...
module.exports = {
    loadConsumerMetaData : function(cb) {
        getConsumers(function(err, consumers){
            if (!!err) {
                logger.trace({err: err}, 'warning will robinson');
                return cb(err);
            }
